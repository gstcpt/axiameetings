...
export function ChatWidget({ meetingId }: ChatWidgetProps) {
    ...
    useEffect(() => {
        const handleUnload = () => {
            navigator.sendBeacon('/api/chat', new Blob(
                [JSON.stringify({ _close: true, sessionId: sessionId.current })],
    ...
    return (
                        <div
                            className={`fixed bottom-4 sm:bottom-6 z-50 ${isRTL ? 'left-4 sm:left-6' : 'right-4 sm:right-6'}`}
                            dir={isRTL ? 'rtl' : 'ltr'}
                        >
                            {/* Chat panel */}
                            <AnimatePresence>
                                            ...
                                            <>
                                                ...
                                                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white" style={{ minHeight: '300px' }}>
                                                    {messages.map((msg, i) => (
                                                        ...
                                                        >
                                                            {/* Avatar */}
                                                            <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center text-white shadow-sm ${msg.role === 'assistant'
                                                            ...
                                                            {/* Bubble */}
                                                            <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-[13px] leading-relaxed transition-all duration-300 ${msg.role === 'assistant'
            ...
            >
                ...
                {/* Pulse ring */}
                {!open && (
                    <span className="absolute inset-[-4px] rounded-full bg-blue-500 animate-ping opacity-20 -z-10" />
                )}
            </motion.button>
        </div>
    );
