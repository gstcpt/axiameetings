...
import { Calendar, MapPin, Video, Play, MoreVertical, Info } from 'lucide-react';
...
export function MeetingCard({ meeting }: MeetingCardProps) {
  ...
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className="bg-white rounded-3xl p-8 border border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all animate-fade-in group relative overflow-hidden"
    >
      <div className="absolute top-0 start-0 w-full h-1 gradient-primary opacity-0 group-hover:opacity-100 transition-opacity" />
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="text-lg font-semibold mt-3 text-slate-900 group-hover:text-primary transition-colors leading-tight" title={meeting.subject}>
            {meeting.subject}
          </h3>
        </div>
      ...
      <div className="flex gap-2">
        <a href={`/meetings/${meeting.id}/live`} className="flex-1 bg-[#002B5B] hover:bg-[#003d80] text-white py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wide transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-900/10 active:scale-95">
          {statusLabel === 'STARTED' ? <Play size={14} fill="currentColor" /> : null}
          {statusLabel === 'STARTED' ? t('card.join') : t('card.start')}
        </a>
        <a href={`/meetings/${meeting.id}`} className="px-4 py-2.5 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 rounded-xl text-xs font-semibold uppercase tracking-wide transition-all text-slate-600 active:scale-95">
          {t('card.details')}
        </a>
      </div>
    </motion.div>
